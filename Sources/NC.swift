...
func humanTurn() {
    selection(chosenMove: moveChoice, isHuman: true)
    drawBoard()
}

func computerTurn() {
    selection(chosenMove: moveChoice, isHuman: false)
    drawBoard()
}

func selection(chosenMove: String, isHuman: Bool) {
    if isHuman == true {
        print("Make your choice: ", terminator: "")
        moveChoice = readLine()?.uppercased() ?? "Choice not made"
    } else {
        // computer automatically makes selection based on logic
        moveChoice = ""
    }
    ...
    case "TL":
        if TL.status != .notSet {
            ...
            isHuman ? selection(chosenMove: moveChoice, isHuman: true) : selection(chosenMove: moveChoice, isHuman: false)
        } else {
            if isHuman {
